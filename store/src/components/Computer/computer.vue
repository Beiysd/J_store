    ...
    export default{
        ...
        data(){
			...
			created(){
					//轮播信息
					this.$nextTick(() => {
						this.axios.get("http://127.0.0.1/sever/computer/computer_lunbo.php")
					.then(res => {
							this.computerLunbo=res.data;
							this.$nextTick(() => {
					...
					//goods_type信息
						...
						.catch(error => {
							console.log(error);
							alert('网络错误computer02，不可访问');
						})

					//goods信息
						this.axios.get("http://127.0.0.1/sever/computer/computer_goods.php")
						.then(res => {
								this.computerGoods=res.data;
								this.listNum = this.computerGoods.length
								console.log('cahng'+this.listNum)
							})
						.catch(error => {
							console.log(error);
							alert('网络错误computer03，不可访问');
						})
					})
				},
        mounted(){
          this.$nextTick(() => {
            ...
            this.computerSwiper.el.onmouseover = function () { //鼠标放上暂停轮播
              this.computerSwiper.autoplay.stop();
            },
              this.computerSwiper.el.onmouseleave = function () {
                this.computerSwiper.autoplay.start();
              }
          })
        },
        watch:{
        	...
        	leftMouseOver(){
        		...
        		this.celan = true
					...
					searchVal(){
						...
						initSwiper(){
							this.computerSwiper = new Swiper('.container1', {
								loop: true,
								autoplay: {
									delay: 2500,
        ...
        computed:{
            ...
            rotationImg(){
                let rotationResult = ''
                rotationResult = this.computerLunbo
...
// 商品栏
            ...
            pageTableData(){
                let pages=Math.ceil(this.computerGoods.length/10);//5为每页设置数量
                let newList=[];
                for(let i=0;i<pages;i++){
                let sonList=[];
                sonList=this.computerGoods.slice(i*10,i*10+10);//5为每页设置数量

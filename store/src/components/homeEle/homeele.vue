...
import Swiper from 'swiper'
    export default{
        ...
        data(){
            return {
				...
				created(){
					//轮播信息
					this.$nextTick(() => {
						this.axios.get("http://127.0.0.1/sever/homele/homeele_lunbo.php")
					.then(res => {
							this.homelLunbo=res.data;
							this.$nextTick(() => {
					...
					//goods_type信息
						...
						.catch(error => {
							console.log(error);
							alert('网络错误homele02，不可访问');
						})

					//goods信息
						this.axios.get("http://127.0.0.1/sever/homele/homeele_goods.php")
						.then(res => {
								this.homeGoods=res.data;
								this.listNum = this.homeGoods.length
								console.log('cahng'+this.listNum)
							})
						.catch(error => {
							console.log(error);
							alert('网络错误homele03，不可访问');
						})
					})
				},
        mounted(){
					this.$nextTick(() => {
						this.homeSwiper.el.onmouseover = function () { //鼠标放上暂停轮播
              this.homeSwiper.autoplay.stop();
            },
						this.homeSwiper.el.onmouseleave = function () {
							this.homeSwiper.autoplay.start();
						}
					})
				},
				watch:{
        ...
        methods:{
          ...
          hiddleTitle(){
            this.titleNum = -1
					...
					initSwiper(){
						this.homeSwiper = new Swiper('.container1', {
              loop: true,
              autoplay: {
                delay: 2500,
        ...
        computed:{
          ...
          rotationImg(){
            let rotationResult = ''
            rotationResult = this.homelLunbo
...
//分页
					pageTableData(){
                let pages=Math.ceil(this.homeGoods.length/10);//5为每页设置数量
                let newList=[];
                for(let i=0;i<pages;i++){
                let sonList=[];
                sonList=this.homeGoods.slice(i*10,i*10+10);//5为每页设置数量

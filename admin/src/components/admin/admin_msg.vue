            ...
            <div class="next_title" v-for="(item,index) in this.goodsList" :key="index">
                <div>
                    ...
                    <div class="ul" v-show="show_num === index">
                        ...
                        <input class="il" name="i_password" type="text"  :value="item.password">
                        <input class="il" name="i_email" type="text"  :value="item.email">
                        <input class="il" name="i_phone" type="text"  :value="item.phone">
...
export default {
    ...
    methods:{
        ...
        succ(val){
            ...
            let f_password = $("input[name='f_password']")[val].value
            let f_email = $("input[name='f_email']")[val].value
            let f_phone = $("input[name='f_phone']")[val].value
            //完成
            let i_id = $("input[name='i_id']")[val].value
            let i_name = $("input[name='i_name']")[val].value
            let i_password = $("input[name='i_password']")[val].value
            let i_email = $("input[name='i_email']")[val].value
            let i_phone = $("input[name='i_phone']")[val].value

            if(!i_id || !i_name || !i_password || !i_email || !i_phone){
                alert('输入不得为空!')
            }else{
                //删除数据
                let del_data = {'id':f_id,'dbId':this.dbId}
                ...
                .then((res)=>{
                    ...
                    console.log(data)
                    this.$http.post("http://127.0.0.1/sever/admin_user/user_succ.php",data)
                    .then((res)=>{
                        let num = res.data
                        if(num == 0){
                            console.log('num--'+num)
                            ...
                            console.log(data)
                            this.$http.post("http://127.0.0.1/sever/admin_user/user_succ.php",add_data)
                            .then((res)=>{})
                        }else{
                            console.log('succe')
                            alert('编辑完成，请刷新页面!')

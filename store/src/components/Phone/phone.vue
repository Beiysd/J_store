        ...
        data(){
            return {
				...
				created(){
					//轮播信息
					this.$nextTick(() => {
						this.axios.get("http://127.0.0.1/sever/phone/phone_lunbo.php")
					.then(res => {
							this.phoneLunbo=res.data;
							this.$nextTick(() => {
					...
					//goods_type信息
						...
						.catch(error => {
							console.log(error);
							alert('网络错误phone02，不可访问');
						})

					//goods信息
						this.axios.get("http://127.0.0.1/sever/phone/phone_goods.php")
						.then(res => {
								this.phoneGoods=res.data;
								this.listNum = this.phoneGoods.length
								console.log('cahng'+this.listNum)
							})
						.catch(error => {
							console.log(error);
							alert('网络错误phone03，不可访问');
						})
					})
				},
        mounted(){
          this.$nextTick(() => {
            this.phoneSwiper.el.onmouseover = function () { //鼠标放上暂停轮播
              this.phoneSwiper.autoplay.stop();
            },
						this.phoneSwiper.el.onmouseleave = function () {
							this.phoneSwiper.autoplay.start();
						}
          })
        },
        watch:{
        ...
        methods:{
        	...
        	leftMouseOver(){
        		...
        		this.celan = true
					...
					initSwiper(){
						this.phoneSwiper = new Swiper('.container1', {
              loop: true,
              autoplay: {
                delay: 2500,
        ...
        computed:{
        	...
        	leftTitle(){
        		...
        		return leftResult
					...
					rotationImg(){
							let rotationResult = ''
							rotationResult = this.phoneLunbo
				...
				// 商品栏
					...
					pageTableData(){
                let pages=Math.ceil(this.phoneGoods.length/10);//5为每页设置数量
                let newList=[];
                for(let i=0;i<pages;i++){
	                let sonList=[];
	                sonList=this.phoneGoods.slice(i*10,i*10+10);//5为每页设置数量

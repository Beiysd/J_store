    ...
    export default{
        ...
        data(){
            return {
				...
				created(){
					//轮播信息
					this.$nextTick(() => {
						this.axios.get("http://127.0.0.1/sever/boy/boy_lunbo.php")
					.then(res => {
							this.boyLunbo=res.data;
							this.$nextTick(() => {
					...
					//goods_type信息
						...
						.catch(error => {
							console.log(error);
							alert('网络错误boy02，不可访问');
						})

					//goods信息
						this.axios.get("http://127.0.0.1/sever/boy/boy_goods.php")
						.then(res => {
								this.boyGoods=res.data;
								this.listNum = this.boyGoods.length
								console.log('cahng'+this.listNum)
							})
						.catch(error => {
							console.log(error);
							alert('网络错误boy03，不可访问');
						})
					})
				},
        mounted(){
          this.$nextTick(() => {
            this.boySwiper.el.onmouseover = function () { //鼠标放上暂停轮播
              this.boySwiper.autoplay.stop();
            },
						this.boySwiper.el.onmouseleave = function () {
							this.boySwiper.autoplay.start();
						}
          })
        },
        watch:{
        ...
        methods:{
            ...
            searchVal(){
						...
						initSwiper(){
							this.boySwiper = new Swiper('.container1', {
              loop: true,
              autoplay: {
                delay: 2500,
...
// 商品栏
            ...
            pageTableData(){
                let pages=Math.ceil(this.boyGoods.length/10);//5为每页设置数量
                let newList=[];
                for(let i=0;i<pages;i++){
                let sonList=[];
                sonList=this.boyGoods.slice(i*10,i*10+10);//5为每页设置数量
